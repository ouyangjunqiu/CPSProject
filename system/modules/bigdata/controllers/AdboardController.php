...
use application\modules\bigdata\model\AdboardRptSourceHistory;
...
class AdboardController extends Controller
{
    public function actionSource(){
        ...
        AdboardRptSourceHistory::model()->deleteAll("logdate=? AND nick=? AND offset=?",array($logdate,$nick,$offset));
        $model = new AdboardRptSourceHistory();
    ...
    public function actionHasget(){
        ...
        $logdate = Env::getRequest("logdate");
        $nick = trim($nick);
        $count = AdboardRptSourceHistory::model()->count("logdate=? AND nick=?",array($logdate,$nick));
