...
class AdzoneRptSource extends Model
{
    /**
     * Returns the static model of the specified AR class.
     * @param string $className active record class name.
     * @return AdzoneRptSource the static model class
     */
    ...
    {
        return '{{zuanshi_adzone_rpt_source}}';
    }
    ...
    public static function fetchAllSummaryByCache($nick){
        try {
            $key = "sys.cache.adzone.summary." . md5($nick);
            if (Cache::check()) {
                $data = Cache::get($key);
                if ($data !== false)
    ...
    public static function fetchAllSummaryByNick($nick){
        ...
        $sources = self::model()->fetchAll("logdate=? AND nick=? ORDER BY offset ASC",array($logdate,$nick));
        $list = array();
        $i = 0;
        foreach($sources as $row) {
            $rpts = json_decode($row["data"],true);

            if(isset($rpts["rptAdzoneDayList"])) {
                foreach ($rpts["rptAdzoneDayList"] as $rpt) {
                    $list[$rpt["adzoneId"]][] = $rpt;
                    $i++;
                }
            }
        ...
        foreach($list as $adboardId => $rpts){
            ...
            foreach($rpts as $rpt){
                $summary["adzoneId"] = $rpt["adzoneId"];
                $summary["adzoneName"] = $rpt["adzoneName"];
                ...
                $summary["cartNum3"] += $rpt["cartNum3"];
                $summary["cartNum7"] += $rpt["cartNum7"];
            }
