...
class DestRptSource extends Model
{
    /**
     * Returns the static model of the specified AR class.
     * @param string $className active record class name.
     * @return DestRptSource the static model class
     */
    ...
    {
        return '{{zuanshi_dest_rpt_source}}';
    }
    ...
    public static function fetchAllSummaryByCache($nick){
        try {
            $key = "sys.cache.dest.summary." . md5($nick);
            if (Cache::check()) {
                $data = Cache::get($key);
                if ($data !== false)
    ...
    public static function fetchAllSummaryByNick($nick){
        ...
        foreach($sources as $row) {
            ...
            if(isset($rpts["rptDestDayList"])) {
                foreach ($rpts["rptDestDayList"] as $rpt) {
