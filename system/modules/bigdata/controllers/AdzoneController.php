...
use application\modules\bigdata\model\AdzoneRptSourceHistory;
...
class AdzoneController extends Controller
{
    public function actionSource(){
        ...
        AdzoneRptSourceHistory::model()->deleteAll("logdate=? AND nick=? AND offset=?",array($logdate,$nick,$offset));
        $model = new AdzoneRptSourceHistory();
    ...
    public function actionHasget(){
        ...
        $logdate = Env::getRequest("logdate");
        $nick = trim($nick);
        $count = AdzoneRptSourceHistory::model()->count("logdate=? AND nick=?",array($logdate,$nick));
